...
try {
		var datasetConsulta = 'DSabertura_vaga';
	...
	try {
		...
		if (dados.constraints != null) {
			for (var i = 0; i < dados.constraints.length; i++) {
				...
				else if (campo == 'NUM_SEQ_ESTADO') setWhere += "\n AND HP.NUM_SEQ_ESTADO " + montaCondConsulta(tipo, like, valorConstraintINI);
				...
				else if (campo == 'UNIDADE') setWhere += "\n AND MP.unidade " + montaCondConsulta(tipo, like, valorConstraintINI);
		...
		myQuery += "\n 		TIMESTAMPDIFF(SECOND, TP.ASSIGN_START_DATE, TP.ASSIGN_END_DATE) AS TEMPO_ATIVIDADE, "
		...
		myQuery += "\n 		'Relatório Abertura Vaga' AS RELATORIO ";
		...
		myQuery += "\n AND PW.COD_DEF_PROCES = 'abertura_vaga'";
		myQuery += "\n AND EP.IDI_TIP_BPMN IN (0, 10, 80, 82, 60) ";
		myQuery += "\n AND EP.NOM_ESTADO NOT IN ('Join', 'Fork') ";
		myQuery += setWhere;
		myQuery += "\n GROUP BY PW.NUM_PROCES, HP.NUM_SEQ_MOVTO "
