...
function retornaDocumentos(dados) {
	try {
		...
		myQuery += "\n 		MP.categoria, ";
		myQuery += "\n 		MP.zEmpresa, ";
		myQuery += "\n 		CASE WHEN MP.Avaliado = 'Muito_satisfeito' THEN 'Muito satisfeito' WHEN  MP.Avaliado = 'Muito_insatisfeito'  THEN 'Muito insatisfeito' ELSE MP.Avaliado END AS 'Avaliado', ";
		...
		while (rs.next()) {
			...
			for (var i = 1; i <= columnCount; i++) {
				...
				// log.dir({ "columnName": columnName, "obj": obj });
				if (null != obj) {
					Arr[i - 1] = rs.getObject(rs.getMetaData().getColumnName(i)).toString();
				} else {
					Arr[i - 1] = "";
